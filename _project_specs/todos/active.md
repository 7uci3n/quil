...
- [ ] `.nvmrc` → `24`.
- [ ] `Dockerfile` builder + runtime stages → `node:24-bookworm` / `-slim`.
- [ ] CI `quality.yml` + `security.yml` → `node-version: "24"`.
- [ ] `@types/node` → `^24` (track runtime, NOT latest 26).
- [ ] `npm rebuild better-sqlite3` against Node 24 ABI.
- **Validation:** on Node 24 — `npm ci && npm run build && npm run lint && npm test`
  all green (263 tests).

### Phase 2 — Safe minor bumps (batch)

- [ ] axios, discord.js, dotenv, zod, tsx, jiti, typescript-eslint, vitest + @vitest/\*.
- [ ] Move `prettier` from `dependencies` → `devDependencies`.
- **Validation:** build + lint + format:check + test all green.

### Phase 3 — Major bumps (one at a time, green suite between each)

- [ ] `eslint@10` + `@eslint/js@10` + `globals@17` (flat config already in use).
- [ ] `lint-staged@16`.
- [ ] `csv-parse@7` — single call site `src/utils/gsheet.ts` (`csv-parse/sync`).
