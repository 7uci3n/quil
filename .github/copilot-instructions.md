...
- **Example**:
  ...
  const check = await db.get(
    `SELECT * FROM pragma_table_info('charlog') WHERE name = 'new_col';`,
  );
  if (!check) {
    await db.exec(
      `ALTER TABLE charlog ADD COLUMN new_col INTEGER NOT NULL DEFAULT 0;`,
    );
  }
  ```
