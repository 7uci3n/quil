...
async function collectUsers(
  ...
  if (multi) {
    for (let i = 1; i <= max; i++) {
      ...
      const c =
        ix.options.getString(`char${i}` as CharacterOptionName) ?? undefined;
      if (u) {
...
async function getEmbedInfo(
  ...
  const embedInfo: embedInfo = { caller };
...
export async function showCharacterEmbed(
  ...
  if (embedInfo.info?.at(0)) {
    ...
    const footer =
      opts.footer ?? "Requested via " + embedInfo.caller.displayName;
    const cp = embedInfo.info.at(0)?.cp ?? 0;
    const gp = (cp / 100).toFixed(2);
    const fields = opts.fields ?? [
      {
        name: "Level",
        value: "⭐ " + String(embedInfo.info.at(0)?.level),
        inline: true,
      },
      {
        name: "Experience (XP)",
        value: "💪 " + String(embedInfo.info.at(0)?.xp),
        inline: true,
      },
      { name: "\u200b", value: "\u200b", inline: true },
      { name: "Gold Pieces (GP)", value: "💰 " + gp, inline: true },
      {
        name: "Golden Tickets (GT)",
        value: "🎫 " + embedInfo.info.at(0)?.tp,
        inline: true,
      },
      {
        name: "Downtime (DTP)",
        value: "🔨 " + embedInfo.info.at(0)?.dtp,
        inline: true,
      },
    ];
    const reply: InteractionReplyOptions = {
      embeds: [
        new EmbedBuilder()
          ...
          .setAuthor({
            name: embedInfo.caller.displayName,
            iconURL: embedInfo.caller.displayAvatarURL(),
          })
          .setTitle(`${title}`)
          .setDescription(desc)
          .addFields(fields)
    ...
    return interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: t("gp.errors.notInSystem", {
        username: embedInfo.caller.username ?? "",
      }),
    });
  }
}

export async function buildMultiEmbed(
