...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sc) =>
    sc.setName("toggle").setDescription("Toggle your “Available to DM” status"),
  )
  .addSubcommand((sc) =>
    sc.setName("list").setDescription("Show who is currently Available to DM"),
  );
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!role) {
    return interaction.reply({
      content: t("dm.roleMissing", { role: DM_ROLE_ID ?? "undefined" }),
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  if (sub === "toggle") {
    ...
    const canToggle =
      ...
      hasAnyRole(
        member,
        [CFG.roles.dm.id, CFG.roles.member.id].filter(Boolean) as string[],
      );
    if (!canToggle) {
      return interaction.reply({
        content: t("dm.toggle.notAllowed"),
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (member.roles.cache.has(role.id)) {
      ...
      return interaction.reply({
        content: t("dm.toggle.disabled", { user: interaction.user.toString() }),
      });
    } else {
      await member.roles.add(role);
      return interaction.reply({
        content: t("dm.toggle.enabled", { user: interaction.user.toString() }),
      });
    }
  }
}
