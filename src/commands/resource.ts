...
const PERMS = {
  ...
  adjust: [ROLE.moderator.id, ROLE.admin.id, ROLE.keeper.id].filter(
    Boolean,
  ) as string[],
...
function addSharedOptions(
  ...
  return sub
    ...
    .addUserOption((o) =>
      o.setName("user").setDescription("Target").setRequired(false),
    )
    .addStringOption((o) =>
      o
        .setName("name")
...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sc) =>
    addSharedOptions(
      sc.setName("show").setDescription("Show resource for a user"),
    ),
  )
  .addSubcommand((sc) =>
    addSharedOptions(
...
export async function execute(ix: ChatInputCommandInteraction) {
  ...
  if (sub === "show") {
    ...
    if (resource === "xp") {
      ...
      const nextDisp =
        next === null ? "—" : `${next.toLocaleString()} XP (to L${level + 1})`;
      const pct =
        next === null
          ? 100
          : Math.floor(((row.xp - curr) / (next - curr)) * 100);
      const pctStr =
        next === null
          ? t("xp.show.fields.max")
          : t("xp.show.progressFmt", {
              pct,
              curr: (row.xp - curr).toLocaleString(),
              range: (next - curr).toLocaleString(),
            });
      fields = [
        {
          name: t("xp.show.fields.level"),
          value: `**${level}**`,
          inline: true,
        },
        {
          name: t("xp.show.fields.xp"),
          value: row.xp.toLocaleString(),
          inline: true,
        },
        {
          name: t("xp.show.fields.prof"),
          value: `+${proficiencyFor(level)}`,
          inline: true,
        },
      ...
      fields = [
        {
          ...
          value: `**${resource === "cp" ? toGp(res) : res}**`,
          inline: false,
        },
      ];
    }
  ...
  const next = await adjustResource(
    user.id,
    [resource],
    [resource === "cp" ? toCp(amt) : amt],
    set,
    row.name,
  );
  if (next) {
    ...
    if (resource === "tp") {
      ...
      resNew = await getPlayerCC(user.id);
    }
    showCharacterEmbed(ix, {
