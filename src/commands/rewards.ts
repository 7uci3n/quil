...
export const data = new SlashCommandBuilder()
  .setName("reward")
  .setDescription(
    "Award XP/GP/GT to players or claim DM/Staff rewards (config-driven).",
  )
  // custom
  .addSubcommand((sc) =>
    sc
      ...
      .addStringOption((o) =>
        o.setName("char1").setDescription("Character #1").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char2").setDescription("Character #2").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char3").setDescription("Character #3").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char4").setDescription("Character #4").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char5").setDescription("Character #5").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char6").setDescription("Character #6").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char7").setDescription("Character #7").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char8").setDescription("Character #8").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o.setName("char9").setDescription("Character #9").setAutocomplete(true),
      )
      .addStringOption((o) =>
        o
          .setName("char10")
          .setDescription("Character #10")
          .setAutocomplete(true),
      )
      .addIntegerOption((o) =>
        o.setName("xp").setDescription("XP to award (>=0)").setMinValue(0),
      )
      .addNumberOption((o) =>
        o.setName("gp").setDescription("GP to award (>=0)").setMinValue(0),
      )
      .addNumberOption((o) =>
        o.setName("gt").setDescription("GT to award (>=0").setMinValue(0),
      )
      .addStringOption((o) =>
        o
          .setName("reason")
          .setDescription("Why? (for audit purposes)")
          .setMaxLength(200),
      ),
  )
  // dm self-claim
  .addSubcommand((sc) =>
    sc
      .setName("dm")
      .setDescription(
        "Claim DM reward for yourself based on your character level.",
      )
      .addBooleanOption((o) =>
        o
          .setName("half")
          .setDescription("Grant half the usual DM rewards")
          .setRequired(false),
      )
      .addStringOption((o) =>
        o.setName("reason").setDescription("Why? (optional)").setMaxLength(200),
      ),
  );
...
export async function execute(ix: ChatInputCommandInteraction) {
  ...
  if (!allowed) {
    await ix.reply({
      ...
      content: t("reward.errors.noPermission"),
    });
    return;
  }
...
async function handleCustom(ix: ChatInputCommandInteraction) {
  ...
  const tpAuto =
    ix.options.getNumber("gt") != null
      ? false
      : ix.options.getNumber("gp") || ix.options.getInteger("xp")
        ? true
        : false;
  ...
  if (!recipients.length) {
    await ix.reply({
      flags: MessageFlags.Ephemeral,
      content: t("reward.errors.noRecipients"),
    });
    return;
  }
  if (!tpAuto && xpIn === 0 && gpIn === 0 && tpIn === 0) {
    await ix.reply({
      flags: MessageFlags.Ephemeral,
      content: t("reward.errors.noInputs"),
    });
    return;
  }
  ...
  for (const [u, c] of recipients) {
    ...
    if (!before) {
      await ix.reply({
        flags: MessageFlags.Ephemeral,
        content: t("reward.errors.userNotInSystem", { username: u.username }),
      });
      return;
    }
    ...
    // Embed
    // field text
    const heading = t("reward.custom.fieldHeading", {
      username: u.displayName,
      charName: before.name,
    });
    const deltaStr = t("reward.custom.fmt.delta", {
      xp: delta.xp ?? 0,
      gp: fmtGp(delta.cp ?? 0),
      gt: tp,
    });
    const beforeStr = t("reward.custom.fmt.before", {
      xp: before.xp.toLocaleString(),
      level,
      gp: fmtGp(before.cp),
      gt: before.tp,
    });
    const afterStr = t("reward.custom.fmt.after", {
    ...
    fields.push({
      name: heading,
      value: t("reward.custom.fieldBody", {
        before: beforeStr,
        after: afterStr,
        delta: deltaStr,
      }),
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({
      text: reason
        ? t("reward.common.footerReason", { reason })
        : t("reward.common.footerDash"),
    });
  ...
  await ix.reply({
    ...
    embeds: [embed],
    allowedMentions: {
      users: Array.from(new Set(recipients.map(([u]) => u.id))),
    },
  });
}
...
async function handleDm(ix: ChatInputCommandInteraction) {
  ...
  if (!before) {
    return ix.reply({
      flags: MessageFlags.Ephemeral,
      content: t("reward.errors.dmNoRecord"),
    });
  }
